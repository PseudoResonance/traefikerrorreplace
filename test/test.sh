...
sed -i "/\"traefik\.http\.routers\.whoami\.entrypoints=web\"/a \ \ \ \ \ \ - \"traefik\.http\.routers\.whoami\.middlewares=${PLUGIN_NAME:-test}@file\"" $DOCKER_COMPOSE
...
if [ ${ENVIRONMENT:=development} = "production" ]; then
	...
	[experimental]
	  [experimental.plugins]
	    [experimental.plugins.${PLUGIN_NAME}]
	...
	[experimental]
	  [experimental.localPlugins]
	    [experimental.localPlugins.${PLUGIN_NAME}]
	      moduleName = "${MODULE_NAME}"
	EOF
